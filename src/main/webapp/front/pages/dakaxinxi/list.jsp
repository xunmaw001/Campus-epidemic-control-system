            ...
            <div class="ng-inmod-ini">
                    <div class="ng-fault ng-container">
                        <div class="ng-inmod-the ng-container">
                            ...
                            <span class="ng-inmod-the-cn" aos="fade-down">打卡信息展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label">打卡所在地</label>
                          <div class="layui-input-inline">
                              <input type="text" name="dakaxinxiDidian" id="dakaxinxiDidian" placeholder="打卡所在地" autocomplete="off" class="layui-input">
                          </div>
                      </div>
     
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label">一月内是否接触过确诊病例</label>
                          <div class="layui-input-inline">
                              <select name="quezhenTypes" id="quezhenTypes" lay-filter="quezhenTypes">
                                  <option value="">--请选择--</option>
                                  <option v-for="item in quezhenTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                              </select>
                          </div>
                      </div>
     
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label">一月内是否接触过疑似病例</label>
                          <div class="layui-input-inline">
                              <select name="yishiTypes" id="yishiTypes" lay-filter="yishiTypes">
                                  <option value="">--请选择--</option>
                                  <option v-for="item in yishiTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                              </select>
                          </div>
                      </div>
     
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label">一月内是否去过中高风险地区</label>
                          <div class="layui-input-inline">
                              <select name="gaofengxianTypes" id="gaofengxianTypes" lay-filter="gaofengxianTypes">
                                  <option value="">--请选择--</option>
                                  <option v-for="item in gaofengxianTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                              </select>
                          </div>
                      </div>
...
<script>
    ...
    // 查询当前字典表相关
        ...
        // 一月内是否接触过疑似病例的查询和初始化
        yishiTypesSelect();
        // 一月内是否去过中高风险地区的查询和初始化
        gaofengxianTypesSelect();
    // 当前表的字典表
        // 一月内是否接触过确诊病例的查询
        function quezhenTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=quezhen_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.quezhenTypesList = res.data.list;
                }
            });
        }
        // 一月内是否接触过疑似病例的查询
        function yishiTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=yishi_types", 'get', {}, function (res) {
                if(res.code == 0){
                    vue.yishiTypesList = res.data.list;
                }
            });
        }
        // 一月内是否去过中高风险地区的查询
        function gaofengxianTypesSelect() {
            http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=gaofengxian_types", 'get', {}, function (res) {
                if(res.code == 0){
        ...
        function pageList() {
            ...
            if (jquery('#dakaxinxiDidian').val() != null && jquery('#dakaxinxiDidian').val() != "" && jquery('#dakaxinxiDidian').val() != "null" ) {
                param['dakaxinxiDidian'] = jquery('#dakaxinxiDidian').val();
            }
         
            var quezhenTypes = $("#quezhenTypes").find("option:selected").val();
            if (quezhenTypes != null && quezhenTypes != "" && quezhenTypes != "null") {
                param['quezhenTypes'] = quezhenTypes;
            }
         
            var yishiTypes = $("#yishiTypes").find("option:selected").val();
            if (yishiTypes != null && yishiTypes != "" && yishiTypes != "null") {
                param['yishiTypes'] = yishiTypes;
            }
         
            var gaofengxianTypes = $("#gaofengxianTypes").find("option:selected").val();
            if (gaofengxianTypes != null && gaofengxianTypes != "" && gaofengxianTypes != "null") {
                param['gaofengxianTypes'] = gaofengxianTypes;
            }
        
            // 获取列表数据
            http.request('dakaxinxi/list', 'get', param, function(res) {
